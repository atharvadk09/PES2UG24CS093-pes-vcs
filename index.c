...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%o %64s %" SCNu64 " %" SCNu64 " %511s",
                   &e->mode, hex, &mtime, &size, path) != 5) {
            fclose(f);
            return -1;
        }
...
int index_save(const Index *index) {
    ...
    // Sort entries by path before writing — use heap allocation to avoid
    // stack overflow (Index struct is large due to fixed-size entry array)
...
// Stage a file: read it, write blob to object store, update index.
// Returns 0 on success, -1 on error.
